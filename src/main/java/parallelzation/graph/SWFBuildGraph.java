...
//        swfDataFlowOutEdges=new ArrayList<>();
    ...
    public SWFNewGraph build(){

        //设置数据节点关系
//        this.swfDataNodeList.removeAll(swfDataNodeList);
        for(SWFDataFlowEdge edge: swfDataFlowEdges){
            if(edge.getLeftNode().getNodeType().equals(NodeType.DATA_NODE)){
                //data -> task
                SWFDataNode dataNode = (SWFDataNode) edge.getLeftNode();
                dataNode.addOutEdge(edge);
//                swfDataNodeList.add(dataNode);
            }else {
                //taks -> data
                SWFDataNode dataNode = (SWFDataNode) edge.getRightNode();
                dataNode.addInEdge(edge);
            }
        }

        SWFNewGraph swfNewGraph = new SWFNewGraph(swfStartNode, swfEndNode, swfDataNodeList, swfTaskNodeList,
                splitControlNodeList, joinControlNodeList, swfControlEdgeList, swfDataFlowEdges);
        return  swfNewGraph;
    }
